...
function vowelCount(str) {
    ...
    return word.reduce(function (accumulator, currentValue) {
        if (vowels.includes(currentValue)) {
            // If the vowel exists in the object accumulator as a key/currentValue, then increase by 1.
            // Otherwise, create key for currentValue in accumulator, assign it the value zero, then increase by 1.
...
function addKeyAndValue(arr, key, value) {
    let arrWithKeyValue = arr.reduce(function (acc, obj) {
        // Set new Object = key value pair followed by key value pairs from obj via the spread operator 
        const newObj = { [key]: value, ...obj };
        //Push newObj into acc which was initialized to an empty array
        acc.push(newObj);
        return acc;
    }, []);
    // Display arrWithKeyValue
    console.log(arrWithKeyValue);
    return arrWithKeyValue;
}
...
function partition(arr, callback) {
    return arr.reduce(function (acc, el) {
        //If callback returns true, push the element el into the first array of acc.
        if (callback(el)) {
            acc[0].push(el);
        }
        //Otherwise, push the element el into the second array of acc.
        else {
            acc[1].push(el);
        }
        return acc;
